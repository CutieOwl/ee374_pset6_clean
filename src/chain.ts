...
import { minerWorker } from "./minerWorker";
...
class ChainManager {
  ...
  async init() {
    ...
    await minerWorker.refreshChaintip(this.longestChainTip.blockid, height);
  ...
  async onValidBlockArrival(block: Block) {
    ...
    if (height > this.longestChainHeight) {
      ...
      await minerWorker.refreshChaintip(block.blockid, height);
